...
def compare_models(
    ...
    from rustic_ml.evaluation.inference import accumulate_inference
    ...
    prev_model = mlflow.pytorch.load_model(
        f"models:/{model_name}/{latest.version}", map_location=device
    )
    ...
    display_comparison_table(
        compute_comparison_metrics(prev_vals),
        compute_comparison_metrics(curr_vals),
        str(latest.version),
    )
...
def compare_note_models(
    ...
    from rustic_ml.evaluation.inference import accumulate_note_inference
    from rustic_ml.mlflow_ui import show_registered_models
...
def compare_note_waveform_models(
    ...
    """Compare a NoteWaveformPredictor against the latest registered version."""
    from rustic_ml.evaluation.inference import accumulate_note_waveform_inference
    from rustic_ml.mlflow_ui import show_registered_models
...
def compare_adsr_models(
    ...
    from rustic_ml.evaluation.inference import accumulate_adsr_inference
    from rustic_ml.mlflow_ui import show_registered_models
    ...
    print(f"Accumulating ADSR inference (notes: {note_label}) ...")
    ...
    print(f"\nCurrent model wins {current_wins}/{len(rows)} metrics vs v{latest.version}.")
    display_adsr_scatter(prev_vals, curr_vals, model_name, str(latest.version))
