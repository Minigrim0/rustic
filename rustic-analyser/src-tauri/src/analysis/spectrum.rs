...
pub fn compute_spectrum(samples: &[f32], sample_rate: u32) -> Vec<Vec<f32>> {
    info!("Computing spectrum for {} samples at {} Hz", samples.len(), sample_rate);
    ...
    for frame_index in 0..num_frames {
        ...
        for i in 0..window_size {
            if start_index + i < samples.len() {
                let window_val = 0.5 * (1.0 - (2.0 * PI * i as f32 / (window_size as f32 - 1.0)).cos());
    ...
    info!("Spectrogram completed: {} frames, {} frequency bins", 
           spectrogram.len(), 
