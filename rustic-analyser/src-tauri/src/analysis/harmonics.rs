...
pub fn identify_peaks(frequencies: &[FrequencyData], threshold: f32, min_distance: f32) -> Vec<FrequencyData> {
...
pub fn analyze_harmonics(peaks: &[FrequencyData], tolerance: f32) -> Vec<Vec<FrequencyData>> {
    ...
    for &fundamental_idx in &[0, 1, 2] {
        ...
        for harmonic_number in 2..10 {
            ...
            if let Some(harmonic) = peaks.iter()
...
mod tests {
    ...
    fn test_analyze_harmonics() {
        ...
        let peaks = vec![
            FrequencyData { frequency: 100.0, magnitude: 1.0, phase: 0.0 },
            FrequencyData { frequency: 198.0, magnitude: 0.7, phase: 0.0 }, // Slightly off 200Hz
            FrequencyData { frequency: 302.0, magnitude: 0.5, phase: 0.0 }, // Slightly off 300Hz
            FrequencyData { frequency: 400.0, magnitude: 0.3, phase: 0.0 },
            FrequencyData { frequency: 150.0, magnitude: 0.2, phase: 0.0 }, // Noise
        ];
