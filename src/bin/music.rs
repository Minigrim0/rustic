...
fn main() {
    ...
    envelope.set_release(1.5, scale * 0.0, Some((0.5, 0.0)));

    let mut other_envelope = Envelope::new(44100.0);
    other_envelope.set_attack(0.0, scale * 1.0, None);
    other_envelope.set_decay(0.05, scale * 0.0, None);
    other_envelope.set_release(0.0, scale * 0.0, None);

    let mut perc = Vec::new();

    for i in 0..(duration as i32) + 1 {
        perc.push(Note::new(TONES_FREQ[NOTES::C as usize][3], i as f64 + 0.75, 0.1).with_generator(GENERATORS::NOISE));
        perc.push(Note::new(TONES_FREQ[NOTES::C as usize][3], i as f64 + 0.9, 0.1).with_generator(GENERATORS::NOISE));
    }
    ...
    notes.append(perc.iter().map(|n| (n, other_envelope.attach(&n.generator))).collect::<Vec<(&Note, Generator)>>().as_mut());
