...
pub fn apply_window(samples: &[f32], window_type: WindowType) -> Vec<f32> {
    ...
    for i in 0..len {
        let window_value = match window_type {
            ...
            WindowType::Hann => 0.5 * (1.0 - (2.0 * std::f32::consts::PI * i as f32 / (len as f32 - 1.0)).cos()),
            WindowType::Hamming => 0.54 - 0.46 * (2.0 * std::f32::consts::PI * i as f32 / (len as f32 - 1.0)).cos(),
            WindowType::Blackman => {
