...
mod tests {
    ...
    fn test_adsr_phase_transition() {
        ...
        assert!(
            attack_mapping < 0.51 && attack_mapping > 0.49,
            "attack mapping should be around 0.5"
        );
        assert_eq!(
            adsr_envelope.attack.get_duration(),
            1.0,
        ...
        let decay_mapping = adsr_envelope
            .decay
            .map_time(adsr_envelope.attack.get_duration(), 1.1);

        assert!(
            decay_mapping < 0.51 && decay_mapping > 0.49,
            "Decay mapping should be around 0.5"
        );
        assert_eq!(
            adsr_envelope.decay.get_duration(),
            0.2,
            "Decay segment should last 0.2 seconds"
        );

        let release_mapping = adsr_envelope.release.map_time(1.0, 1.5);

        assert!(
            release_mapping < 0.51 && release_mapping > 0.49,
            "Release mapping should be around 0.5"
        );
        ...
        assert!(
            mid_attack > 0.49 && mid_attack < 0.51,
            "Center value of the attack should be around 0.5 (reality {})",
            mid_attack
        );
        assert!(
            end_attack > 0.99 && end_attack < 1.01,
            "Attack should end around 1.0 (reality: {})",
            end_attack
        );
        assert!(
            mid_decay > 0.89 && mid_decay < 0.91,
            "Middle of the decay segment should be 0.9 (reality: {})",
            mid_decay
        );
        assert!(
            end_decay > 0.79 && end_decay < 0.81,
            "End of the decay segment should be 0.8 (reality: {})",
            end_decay
        );
    }
